  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="rounded-lg border border-line bg-panel shadow-card grid grid-cols-3 divide-x divide-line">
        ...
        <div className="px-5 py-4">
          <div className={`text-sm font-medium mt-1.5 ${highRiskOwned > 0 ? "text-alarm" : "text-accent"}`}>
      ...
      <div>
        ...
        <div className="rounded-md border border-line bg-panel shadow-card divide-y divide-line">
          {person.usages.map((u) => (
            <Link key={u.id} href={`/assets/${u.aiAssetId}`} className="flex items-center justify-between px-4 py-3 text-sm hover:bg-black/[0.015] transition-colors">
        ...
        <div>
          ...
          <div className="rounded-md border border-line bg-panel shadow-card divide-y divide-line">
            {recentActivity.map((a) => (
              <Link key={a.id} href={`/activity/${a.id}`} className="flex items-center justify-between px-4 py-3 text-sm hover:bg-black/[0.015] transition-colors">
