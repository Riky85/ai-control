...
export default async function SetupCard({ orgId }: { orgId: string }) {
  ...
  const steps = [
    { title: "Connect a provider", hint: "Paste an API key, connect GitHub or import a CSV.", done: connected > 0 || githubOrKeys > 0, href: "/connectors", cta: "Connect" },
    { title: "Review what was found", hint: "Approve or reject each AI system.", done: reviewed > 0, href: "/governance?tab=reviews", cta: "Review" },
    { title: "Assign owners", hint: "Every system needs someone responsible.", done: owned > 0, href: "/assets", cta: "Assign" },
